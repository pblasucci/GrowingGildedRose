...
{
    private static int StandardRamp(int sellIn) => sellIn <= 0 ? 2 : 1;
    ...
    (
        ...
        item.Update(sellIn => sellIn - (int)totalDays, StandardRamp) switch
        {
    ...
    (
        ...
        item.Update(sellIn => sellIn - 1, StandardRamp) switch
        {
            Depreciating (_, var quality, _) =>
    ...
    [Property]
    public Property
        After_1_day_backstage_pass_has_no_quality_if_SellIn_is_negative
        (
            PositiveInt worth
        )
    {
        ...
        return item.Update(sellIn => sellIn - 1, StandardRamp) switch
        {
    ...
    [Property]
    public Property
        After_1_days_backstage_pass_has_quality_reduced_appropriately
        (
            BackstagePass item
        )
    {
        ...
        static Property ExpectedWorth(Quality one, Quality two, int target)
        {
            ...
            return okay.Label(
                $"{one} - {two} ≠ {target} ∧ ¬{Quality.MaxValue}");
        }
    }
}
